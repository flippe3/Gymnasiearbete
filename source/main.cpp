...
int main(int argc, char *argv[])
{
    ...
    GLFWwindow* window = glfwCreateWindow(SCREEN_WIDTH, SCREEN_HEIGHT, "Gymnasiearbete", nullptr, nullptr);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if(Project.resize)
        {
            ...
            glfwSetWindowSize(window, Project.Size_Width, Project.Size_Height);
            window_resize_handler(window, Project.Size_Width, Project.Size_Height);
    ...
    */
    ...
    if (key >= 0 && key < 1024)
    {
        if (action == GLFW_PRESS)
            Project.Keys[key] = GL_TRUE;
        else if (action == GLFW_RELEASE)
            Project.Keys[key] = GL_FALSE;
    }
}
